...
# Absolute Biomass
tab5b <- get.par("logB",res,exp=TRUE)
...
tab6b_<- tab6[grep(".",rownames(tab6b), fixed=TRUE, invert=TRUE),] 
...
#catch <- as.data.frame(get.par("logCpred", res$man$`F=Fmsy_C_fractile`, exp = T))
catch <- as.data.frame(get.par("logCpred", res, exp = T))
...
res <- manage(res, scenarios=c(4,2,3,8), maninterval = c(2025, 2026), intermediatePeriodCatch = TAC2)
...
Fsq_value <- get.par("logFFmsy", res$man[[1]], exp = T)[as.character(2024-0.0625), 2]
Fint_value <- get.par("logFFmsy", res$man[[1]], exp = T)[as.character(2025-0.0625), 2]
res <- add.man.scenario(res, "F=Fsq_ffac", ffac = Fsq_value/Fint_value, intermediatePeriodCatch = TAC2)
...
# add ffmsy
sg_df <- cbind(sg_df,  tab6_3[c(1:(nrow(tab6_3)-2)),c(2:4) ])
...
ffmsy_int <- get.par("logFFmsy", res$man[[1]], exp = T)[as.character(max.year-1.0625), 2]
...
prev.advice <- 1115 ## needs to be manually updated
...
Opt_ffmsy <- c(get.par("logFFmsy", res$man$ices, exp = T)[as.character(max.year-0.0625), 2],
               get.par("logFFmsy", res$man$Fmsy, exp = T)[as.character(max.year-0.0625), 2],
               get.par("logFFmsy", res$man$currentF, exp = T)[as.character(max.year-0.0625), 2],
               get.par("logFFmsy", res$man$noF,  exp = T)[as.character(max.year-0.0625), 2],
               get.par("logFFmsy", res$man$`F=Fsq_ffac`,  exp = T)[as.character(max.year-0.0625), 2])
